...
churn_train_model <- C5.0(churn_train[-11], churn_train$Exited,
                    control = C5.0Control(minCases = 400))

#display simple facts about the tree
summary(churn_train_model)

#plot the tree
plot(churn_train_model)

#Evaluatingmodel performance with the test
#create a factor vector of predictions on test data
churn_pred<-predict(churn_train_model,churn_test)
